document.addEventListener('DOMContentLoaded', () => {
    ...
    function render() {
        ...
        if (elements.liveStatus) {
            ...
            if (window.NETWORK_OFFLINE) {
                ...
                if (liveCount > 0) {
                    elements.liveStatus.innerHTML = `<span class="source-dot live"></span> Modo: TIEMPO REAL(Finnhub) - ${
    { liveCount } activos en vivo <span style = "margin-left:8px; opacity:0.6; font-size:0.9em"> (Sinc: ${ lastSyncTime })</span> `;
    ...
    function addMoreFromStockByTicker(ticker) {
        ...
        if (stock) {
            ...
            elements.dateInput.valueAsDate = new Date();
            elements.modalTitle.textContent = `Añadir Inversión - ${
            { ticker } `;
            toggleModal(true);
        }
    }
